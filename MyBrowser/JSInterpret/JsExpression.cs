...
namespace JSInterpret
{
    public static class CoreInterpreter
    {
        ...
        {
            Int64 num;
            if (Int64.TryParse(x.value, out num))//bug !!!if get "1",it will return num 1
            {
                return num;
            }
            else if (x.value[0] == '\'')
            ...
            else if (x.value.Equals("true") || x.value.Equals("false")) // if it is just string "true" or "false"?
            {
    ...
    public class JsExpression : JsObject
    {
        public string value { get; set; }//迟早要改为Token类型，否则在解释的时候无法判定基本类型如何转化
        ...
        public JsObject evaluate(Env env)
        {
            ...
            else
            {
                ...
                // 添加一种类型 dict !!! x.value == "dict"
                // 添加数组类型 list !!! x.value == "list"
