    ...
    public static partial class JavaScript
    {
        ...
        public static JsExpression ParseFunction(List<Token> list)
        {
            ...
            if (list[index].type == TokenType.IDENTIFY)//mathc function name
            {
                ...
                index++;//match (
                (list[index].type == TokenType.OpenParenthese).OrThrows(list[index].line + ":" + list[index].column + 
                    "expected an open parenthese");
                index++;//match args
            }
            ...
            {
                exceptions.Add(new Exception(list[index].line + ":" + list[index].column + 
                    "syntax error,expected an identifier or an open parenthese"));
            }
            ...
            while (list[index].type != TokenType.CloseParenthese)//add args
            {
                ...
                (list[index].type == TokenType.COMMA).OrThrows(list[index].line + ":" + list[index].column + 
                    "expected a comma");
                index++;
            }
            funcExp.child.Add(argsExp);
            index++;
            (list[index].type == TokenType.OpenBrace).OrThrows(list[index].line + ":" + list[index].column + 
                "expected a OpenBrace");
            funcExp.child.Add(ParseExpList(list));
            (list[index].type == TokenType.CloseBrace).OrThrows(list[index].line + ":" + list[index].column + 
                "expected a CloseBrace");
        ...
        public static JsExpression ParseReturn(List<Token> list)
        {
            ...
            index++; // match ;
            (list[index].type == TokenType.SemiColon).OrThrows(list[index].line+":"+ list[index].column + 
        ...
        public static JsExpression ParseCompareExpression(List<Token> list)
        {
            if(list[index].type == TokenType.NUMBER      || 
                ...
                list[index].type == TokenType.TRUE       ||
                list[index].type == TokenType.FALSE
                )
            {
        ...
        public static bool isSingleExpression(List<Token> list)
        {
            return list[index].type == TokenType.TRUE ||
                list[index].type == TokenType.FALSE ||
                list[index].type == TokenType.IDENTIFY ||
                list[index].type == TokenType.NUMBER ||
                list[index].type == TokenType.NULL ||
                list[index].type == TokenType.UNDEFINED;
        }
        public static JsExpression GetCompoundExpressionItem(List<Token> list)
        {
            ...
            else if (isSingleExpression(list))// Number or Math Exp
            {
                return new JsExpression(list[index].name, current);
            }
            return result;
        ...
        {
            JsExpression result = null;
            if (isBoolOperation(list[index + 1]))
            {
                return ParseBool(list);
            }
            else if (isSingleExpression(list))// Number or Math Exp
        ...
        public static JsExpression ParseUnaryBoolExpression(List<Token> list)
        {
            ...
            current = result;
            index++;// match next
            if (isSingleExpression(list))// match !singleExp
            {
        ...
        public static JsExpression ParseAssign(List<Token> list)
        {
            ...
            index++;
            (list[index].type == TokenType.BIND).OrThrows(list[index].line + ":" + list[index].column + 
                "expected '='");
            index++;
        ...
        public static JsExpression ParseIf(List<Token> list)
        {
            ...
            index++;
            (list[index].type == TokenType.OpenParenthese).OrThrows(list[index].line + ":" + list[index].column + 
                "expected a OpenParenthese");
            index++;
            ifExp.child.Add(ParseBool(list));//parse bool expression
            index++;
            (list[index].type == TokenType.CloseParenthese).OrThrows(list[index].line + ":" + list[index].column + 
                "expected a CloseParenthese");
            index++;
            (list[index].type == TokenType.OpenBrace).OrThrows(list[index].line + ":" + list[index].column + 
                "expected a OpenBrace");
        ...
        public static JsExpression ParsePointExpression(List<Token> list)
        {
            ...
            index++;//match .
            (list[index].type == TokenType.POINT).OrThrows(list[index].line + ":" + list[index].column + 
                "expected a Point.");
            current.child.Add(new JsExpression(objName, current));
            index++;
            (list[index].type == TokenType.IDENTIFY).OrThrows(list[index].line + ":" + list[index].column + 
                "behind point expression needs a identifier");
            ...
            for (; index < list.Count; index++)
            {
                ...
                switch (item.type)
                {
                    ...
                    case TokenType.OpenParenthese:
                        ...
                        program.child.Add(current);
                        break;
                    case TokenType.UNDEFINED:// bug 需要更改bool类型与这两种类型的布尔运算的结果
                    case TokenType.NULL:   // bug
                    case TokenType.TRUE:
                    ...
                    case TokenType.NUMBER: // num | Math Exp | Bool Exp 
                        current = GetCompoundExpressionItem(list);
                        index++;//match ;
                        program.child.Add(current);
                        break;
                    case TokenType.STRING:
                        ...
                        break;
                    default:
                        exceptions.Add(new Exception(current.value+"-->" + list[index].line + ":" + list[index].column + 
                            " is an unknow expression"));
                        break;
                }
                current = program;
            }
