...
namespace JSInterpret
{
    ...
    public class JsNumber : JsObject
    {
        ...
        public static bool operator ==(JsNumber num1, JsNumber num2)//果然是要重载==运算符能比较相等的数
        {
    ...
    {
        private List<JsObject> items;
        private Dictionary<JsObject, JsObject> dict;
        private Env scope;
        public JsArray()
        {
            items = new List<JsObject>();
            dict = new Dictionary<JsObject, JsObject>();
            scope = new Env();
        }
        public JsArray(JsObject[] _items, Env _scope)
        {
            items = ((JsObject[])_items.Clone()).AsEnumerable() as List<JsObject>;
            scope = _scope;
        }
        /// 索引器
        public JsObject this[int index]
        {
            get
            {
                bool condition = index >= 0 && index < items.Count;
                condition.OrThrows(this.ToString() + "index out of range");
                if (condition)
                {
                    return items[index];
